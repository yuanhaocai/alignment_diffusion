...
from .model import *
import pandas as pd
# The fitting workflow injects its packed training/validation dataset.
Dataset_text_tabular = None


def compute_text_sup_loss(core_model, text_proj, y, is_regression,
                          y_class_weight, text_pool_for_sup, weight_sup_loss_text):
    """Supervised loss on the (pooled) text projection via core_model.head_text.

    text_proj: [B, n_tokens, emb_dim]. Returns a scalar tensor. Skips work
    (returns 0) when weight_sup_loss_text == 0, so default behaviour is unchanged.
    """
    if weight_sup_loss_text == 0.0:
        return text_proj.new_zeros(())
    if text_pool_for_sup == "first":
        pooled = text_proj[:, 0, :]
    else:
        pooled = text_proj.mean(dim=1)
    pred_text = core_model.head_text(pooled)
    if is_regression:
        return torch.nn.functional.mse_loss(pred_text.squeeze(-1), y)
    return torch.nn.functional.cross_entropy(pred_text, y, weight=y_class_weight)
...
def main(args):
    ...
    for epoch in range(args.n_epochs):
        ...
        for batch in tqdm(train_loader, disable=not accelerator.is_main_process):
            ...
            pred = unwrap_model(model).head(tab_dat_proj)
            if is_regression:
            ...
            sup_loss_text = compute_text_sup_loss(
                unwrap_model(model), text_proj, y, is_regression,
                y_class_weight, args.text_pool_for_sup, args.weight_sup_loss_text
            )

            loss = (args.weight_clip_loss * loss_dat_text
                    + args.weight_sup_loss * sup_loss
                    + args.weight_sup_loss_text * sup_loss_text)
            ...
            sup_text_losses.append(accelerator.gather_for_metrics(sup_loss_text.detach()))
        ...
        sup_text_loss = torch.cat(sup_text_losses).float().mean().item()
        loss_dict = {
            'train_loss': float(train_loss),
            'clip_loss': float(dt_loss),
            'sup_loss': float(sup_loss),
            'sup_text_loss': float(sup_text_loss),
        }
    ...
    if accelerator.is_main_process:
        ...
        train_loss_log = pd.DataFrame(
            train_loss_log,
            columns=['train_loss', 'clip_loss', 'sup_loss', 'sup_text_loss']
        )
