...
class ResidualProjector(torch.nn.Module):
    def __init__(self, emb_dim=768, residual_scale=1.0, zero_init=True):
        ...
        # Keep the fixed scale in the checkpoint so transformation cannot
        # silently use a value different from the one used during training.
...
def masked_token_mean(text):
    """Mean-pool non-padding token rows.

    Padding positions are stored as exact zeros. Embeddings without zeroed
    padding are treated as having all token positions valid.
    """
    if text.ndim != 3:
        raise ValueError(f"expected [batch, tokens, dim], got {tuple(text.shape)}")
    mask = text.norm(dim=-1).gt(0).to(text.dtype)
    denominator = mask.sum(dim=1, keepdim=True).clamp_min(1.0)
    return (text * mask.unsqueeze(-1)).sum(dim=1) / denominator
...
class TabTextAlign(torch.nn.Module):
    def __init__(
        ...
        if y_type == "categorical":
            ...
            out_dim = 1
        # The response-aware predictor is applied to the normalized,
        # concatenated tabular and pooled-text representations, matching c_i in
        # the paper and allowing the supervised loss to update both projectors.
        self.head = torch.nn.Linear(2 * emb_dim, out_dim)
    ...
    def response_condition(self, tab_proj, text_proj):
        tab_condition = F.normalize(tab_proj, p=2, dim=-1, eps=1e-8)
        text_condition = F.normalize(
            masked_token_mean(text_proj), p=2, dim=-1, eps=1e-8
        )
        return torch.cat([tab_condition, text_condition], dim=-1)

    def predict_response(self, tab_proj, text_proj):
        return self.head(self.response_condition(tab_proj, text_proj))
